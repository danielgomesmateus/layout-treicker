...
Plugin.call($spy,data)})})}(jQuery);$('document').ready(function(){$('#cep_from, #cep_destiny').keyup(function(){var value=$(this).val();var input=$(this).attr('id');if(value.length==8){$(this).searchAddress(value,input);}});$.fn.searchAddress=function(cep,input){$.get("https://viacep.com.br/ws/"+cep+"/json/",function(data){if(data.erro&&data.erro===true){return;}
var address=data.logradouro+", "+data.bairro+", "+data.localidade+", "+data.uf+" - "+data.cep;switch(input){case'cep_from':$('#address_from').val(address);break;case'cep_destiny':$('#address_destiny').val(address);break;}});};});